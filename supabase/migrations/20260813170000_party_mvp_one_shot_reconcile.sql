-- ONE-SHOT PARTY MVP RECONCILIATION
-- Run only this file for the current Party database state.
-- It is atomic: any failure rolls back the entire repair.
...
-- Final authoritative Party action RPCs.
...
do $reconcile_verification$
declare
  v_total integer;
  v_personality integer;
  v_attempt integer;
  v_count integer;
  v_itemless_attempt integer;
  v_itemless_count integer;
  v_start_definition text;
  v_finish_definition text;
begin
  select
    count(*),
    count(*) filter (where challenge_type = 'choice' and category = 'personality'),
    count(*) filter (where challenge_type = 'attempt' and category = 'attempt'),
    count(*) filter (where challenge_type = 'count' and category = 'count'),
    count(*) filter (
      where challenge_type = 'attempt'
        and cardinality(coalesce(required_items, '{}'::text[])) = 0
    ),
    count(*) filter (
  ...
  into
    v_total, v_personality, v_attempt, v_count,
    v_itemless_attempt, v_itemless_count
  from public.party_challenges
  where enabled;
  ...
  if v_total <> 60
     ...
     or v_attempt <> 20
     or v_count <> 20
     or v_itemless_attempt <> 0
     or v_itemless_count <> 17 then
    raise exception
      'Final Party deck mismatch: total %, personality %, attempt %, count %, itemless attempt %, itemless count %',
      v_total, v_personality, v_attempt, v_count,
      v_itemless_attempt, v_itemless_count;
  end if;
  ...
  if exists (
    ...
    raise exception 'An enabled legacy binary challenge remains';
  end if;
  ...
  if v_finish_definition not ilike
     '%Only the host can finish an Attempt%'
     or v_finish_definition not ilike
     '%and not v_player.is_host%' then
    raise exception 'Party early-finish ownership guard is incomplete';
  end if;
end;
...
-- attempt/attempt 20 (0 itemless, duration 0)
-- choice/personality 20 (20 itemless)
-- count/count 20 (17 itemless, duration 60)
select
  challenge_type,
  category,
  count(*) as enabled_cards,
  count(*) filter (
    where cardinality(coalesce(required_items, '{}'::text[])) = 0
  ) as itemless_cards,
...
from public.party_challenges
where enabled
group by challenge_type, category
